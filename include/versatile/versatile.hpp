...
using enable_default_constructor_t = enable_default_constructor< (std::is_default_constructible_v< alternative_types > || ...) >;

template< typename ...alternative_types >
struct versatile
        : enable_default_constructor_t< alternative_types... >
{

    using default_index = index_of_default_constructible< alternative_types..., void >;
...
private :
    ...
    template< typename ...types >
    using dispatcher_t = dispatcher< (std::is_trivially_destructible_v< alternative_types > && ...), (std::is_trivially_default_constructible_v< alternative_types > && ...), alternative_types... >;

    using enabler = enable_default_constructor_t< alternative_types... >;
    using storage = dispatcher_t< alternative_types... >;
...
public :
    ...
    which() const noexcept
    {
        if (storage_.which_ == std::size_t{}) { // is trivially default constructed?
            return sizeof...(alternative_types); // if so, then it always point to the first (leftmost) alternative type
        }
        return storage_.which_;
    }

    template< typename type >
    using index_at_t = index_at_t< unwrap_type_t< type >, unwrap_type_t< alternative_types >..., void >;
    ...
    template< std::size_t i = sizeof...(alternative_types), typename ...arguments >
    constexpr
    void
